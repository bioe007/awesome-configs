...
local AMIX_MASTER = "amixer get Master"
...
function volume_adjust(inc)
        ...
        local s_mix = "amixer get Master | grep -om1 '[[:digit:]]*%' | tr -d %"
        local volume = tonumber(awful.util.pread(s_mix))

        local is_muted = string.find(awful.util.pread(AMIX_MASTER),
                                                      '%[on%]') == nil
        if is_muted then volume = 0 end
        vol_notification = fancy_notify(volume,
                                        volume_get_icon,
                                        vol_notification)
end
