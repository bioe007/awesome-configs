...
--{{{MOCP, FS and BATTERY
widgets.mocp = mocp.init({iconpath = settings.theme_path.."/music/sonata.png",
                          width = 120,
})
...
widgets.battery = battery.init({
    colors = {
        warn = beautiful.fg_batt_warn,
        crit = beautiful.fg_batt_crit,
        low  = beautiful.fg_batt_low,
        mid  = beautiful.fg_batt_mid,
        norm = beautiful.fg_sb_hi
    },
    limits = {
        warn = 10,
        crit = 25,
...
for s = 1, screen.count() do
    ...
    widgets.layoutbox[s]:buttons(awful.util.table.join(
        awful.button({}, 1, function() awful.layout.inc(settings.layouts, 1)
        end),
        awful.button({}, 3, function() awful.layout.inc(settings.layouts, -1)
        end),
        awful.button({}, 4, function() awful.layout.inc(settings.layouts, 1)
        end),
        awful.button({}, 5, function() awful.layout.inc(settings.layouts, -1)
        end)
        )
    )
