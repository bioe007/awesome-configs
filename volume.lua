...
function vol(mode, percent)
    --{{{ the workhorse fx
    if mode == "update" then
        local fd = io.popen("amixer -c " .. config.cardid .. " -- sget " ..
                            config.channel)
        ...
        if string.find(status, "on", 1, true) then
            config.widget:set_gradient_colors({beautiful.fg_sb_hi or
                                                beautiful.fg_focus,
                                                beautiful.fg_focus})
        else
            config.widget:set_gradient_colors({beautiful.fg_normal,
                                                beautiful.fg_normal})
        end
        -- config.widget.text = volume
        config.widget:set_value(volume/100)
    elseif mode == "up" then
        awful.util.spawn("amixer -q -c " .. config.cardid .. " sset " ..
                        config.channel .. " " .. (percent or 5) .. "%+", false)
        vol("update")
    elseif mode == "down" then
        ...
        awful.util.spawn("amixer -c " .. config.cardid .. " sset " ..
                            config.channel .. " toggle", false)
        vol("update")
    end
end
--}}}
